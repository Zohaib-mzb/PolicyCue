...
function Analyzer({ onComplete, initialTab }: { onComplete: (analysis: ActiveAnalysis) => void; initialTab: AnalysisType }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      if (tab === 'website') { const result = await ingestWebsite(url.trim(), controller.current.signal); onComplete({ type: 'website', documentId: result.document_id, sessionToken: result.session_token, identity: result.url, chunks: result.chunks, result }) }
      else if (tab === 'pdf' && file) { const result = await ingestPdf(file, controller.current.signal); onComplete({ type: 'pdf', documentId: result.document_id, sessionToken: result.session_token, identity: result.filename || file.name, chunks: result.chunks, result }) }
      else { const result = await ingestText(text, title, controller.current.signal); onComplete({ type: 'text', documentId: result.document_id, sessionToken: result.session_token, identity: result.title, chunks: result.chunks, result }) }
...
function CoverageNotice({ result, onRecovery, busy }: { result: WebsiteIngestion; onRecovery: (type: AnalysisType) => void; busy: boolean }) {
  ...
  return <div className={`coverage-notice ${notice.tone}`} role="status"><div><strong>{notice.title}</strong><p>{notice.body}</p></div>{needsRecovery && <div className="recovery-actions" aria-label="Other ways to analyze"><button type="button" onClick={() => onRecovery('website')} disabled={busy}>Direct policy URL</button><button type="button" onClick={() => onRecovery('text')} disabled={busy}>Paste text</button><button type="button" onClick={() => onRecovery('pdf')} disabled={busy}>Upload PDF</button></div>}</div>
}
...
function Workspace({ analysis, onReset, onRecovery }: { analysis: ActiveAnalysis; onReset: () => void; onRecovery: (type: AnalysisType) => void }) {
  ...
  const ask = async (event: FormEvent) => { event.preventDefault(); if (!question.trim() || asking || !canAsk) return; setAsking(true); setError(''); try { setAnswer(await askQuestion(analysis.documentId, question.trim(), analysis.sessionToken)); setQuestion('') } catch (requestError) { setError(friendlyError(requestError)); if (requestError instanceof ApiError && requestError.status === 404) setAnswer(null) } finally { setAsking(false) } }
  const beginNewAnalysis = async () => { setDeleting(true); setError(''); try { await deleteAnalysis(analysis.documentId, analysis.sessionToken); onReset() } catch (requestError) { setError(friendlyError(requestError)); setDeleting(false) } }
  const beginRecovery = async (type: AnalysisType) => { if (deleting) return; setDeleting(true); setError(''); try { await deleteAnalysis(analysis.documentId, analysis.sessionToken); onRecovery(type) } catch (requestError) { setError(friendlyError(requestError)); setDeleting(false) } }
  return <motion.section className="workspace" aria-labelledby="workspace-title" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}>
    ...
    {website && <CoverageNotice result={website} onRecovery={beginRecovery} busy={deleting}/>} 
